...
namespace nuc {
     ...
     *    writer @a dest.
     */
    task_queue::task_type make_move_task(std::shared_ptr<dir_type> src_type, const std::vector<dir_entry*> &entries, const pathname &dest);
     ...
     */
	void move(cancel_state &state, const std::vector<pathname> &paths, const pathname &dest, dir_writer &dir);
}
